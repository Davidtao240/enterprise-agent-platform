...
func TestEngineParseDefinitionRejectsUnknownEdgeTarget(t *testing.T) {
	engine := NewEngine()
	_, err := engine.ParseDefinition(`{
		"nodes": [
			{"id":"upload","type":"file_upload","name":"Upload"}
		],
		"edges": [
			{"from":"upload","to":"missing"}
		]
	}`)
	if err == nil {
		t.Fatal("expected parse error for edge pointing to missing node, got nil")
	}
}
