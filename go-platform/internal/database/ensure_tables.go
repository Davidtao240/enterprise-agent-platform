...
// EnsureCriticalTables 确保关键表存在，即使迁移失败也能正常工作。
// 这是一个兜底方案，用于处理迁移链中断的情况。
func EnsureCriticalTables(ctx context.Context, db Execer) error {
	statements := []string{
		// PostgreSQL Roles (for GRANT statements in migrations)
		`DO $$ BEGIN
			IF NOT EXISTS (SELECT FROM pg_roles WHERE rolname = 'platform_admin') THEN CREATE ROLE platform_admin; END IF;
			IF NOT EXISTS (SELECT FROM pg_roles WHERE rolname = 'finance_manager') THEN CREATE ROLE finance_manager; END IF;
			IF NOT EXISTS (SELECT FROM pg_roles WHERE rolname = 'finance_user') THEN CREATE ROLE finance_user; END IF;
			IF NOT EXISTS (SELECT FROM pg_roles WHERE rolname = 'ops_viewer') THEN CREATE ROLE ops_viewer; END IF;
		END $$;`,

		// Agent Package Tables
		...
		// Add missing columns to agent_run_logs
		...
		// Fix role permissions for ops_viewer
		`INSERT INTO role_permissions (role_id, permission_id)
			SELECT '20000000-0000-0000-0000-000000000004', p.id
			FROM permissions p
			WHERE p.code IN ('agent:read', 'tool:read', 'trace:read', 'observability:read', 'outbox:read')
			AND NOT EXISTS (
		...
		// Fix role permissions for finance_user
		`INSERT INTO role_permissions (role_id, permission_id)
		...
		// Fix role permissions for finance_manager
		`INSERT INTO role_permissions (role_id, permission_id)
	...
	for _, stmt := range statements {
		if _, err := db.Exec(ctx, stmt); err != nil {
			log.Printf("[database] EnsureCriticalTables warning: %v", err)
		}
	}

	log.Println("[database] Critical tables verified")
	return nil
}

// Execer 接口用于执行 SQL 语句，与 pgxpool.Pool 兼容
